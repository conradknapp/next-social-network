...
const decode = ({ token }) => {
  ...
  const { _id, email, type } = token || {};
  return { user: { _id, email, type } };
};
...
export const followUser = async (userId, followId) => {
  const { data } = axios.put("/api/users/follow", { userId, followId });
  console.log(data);
  return data;
};

export const unfollowUser = async (userId, unfollowId) => {
  const { data } = axios.put("/api/users/unfollow", { userId, unfollowId });
  console.log(data);
  return data;
};
