...
export const authInitialProps = protectedRoute => ({
  req,
  res,
  query: { userId }
}) => {
  ...
  if (protectedRoute && isAnonymous && currentPath !== "/signin") {
...
export const signupUser = async signupUserPayload => {
  const { data } = await axios.post("/api/auth/signup", signupUserPayload);
  return data;
};

export const signinUser = async signinUserPayload => {
  const { data } = await axios.post("/api/auth/signin", signinUserPayload);
  if (typeof window !== "undefined") {
...
export const followUser = async followUserPayload => {
  const { data } = await axios.put("/api/users/follow", followUserPayload);
  return data;
};

export const unfollowUser = async unfollowUserPayload => {
  const { data } = await axios.put("/api/users/unfollow", unfollowUserPayload);
  return data;
};
...
export const listPostFeed = async userId => {
